...
unsigned g_FAILED_FRAMES{ 0 };
unsigned g_SUCCEDED_FRAMES{ 0 };
    ...
    void InfraredFrameArrived(IInfraredFrameArrivedEventArgs* pArgs)
    {
        ...
        if (SUCCEEDED(hr)){
            ...
            if (SUCCEEDED(hr)){
                ...
                g_SUCCEDED_FRAMES++;
                TRACE(L"Total succeded frames: %d", g_SUCCEDED_FRAMES);
            ...
            {
                g_FAILED_FRAMES++;
                TRACE(L"Failed to acqurie frame!");
                TRACE(L"Total failed frames: %d", g_FAILED_FRAMES);
            }
